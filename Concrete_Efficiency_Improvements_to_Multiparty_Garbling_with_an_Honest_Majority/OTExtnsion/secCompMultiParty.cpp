...
// //**************************//
// unsigned long* rCounterPartyLC;
__m128i* privateKeysSent;
__m128i* privateKeysReceived;
// //**************************//
...
//***************************//
__m128i LoadSeedLatinCrypt(int party, int gate, int row, int location)
{

	__m128i temp=_mm_set_epi32(repetition,gate,row,location);
	__m128i temp2 = _mm_xor_si128(privateKeysSent[party],temp);
	__m128i temp3 = _mm_xor_si128(privateKeysReceived[party],temp);

	AES_encryptC(&temp2,&temp2,&fixed_aes_key);

	AES_encryptC(&temp3,&temp3,&fixed_aes_key);
...
void printVersion(int version)
{
	...
	if (versionString[0]!='u')
	{
		if (version<10)
			std::cout<<", all evaluating." << std::endl;
		else
			std::cout<<", one evaluator." << std::endl;
	}
	else
		std::cout<<"This version is unrecognized - do you know what you are doing? (Are you sure?)"<< std::endl;
